...
int Program::run(int *eip) {
  ...
  while (true) {
    ...
    for (auto d = 0u; d < stack.size(); d++) {
      ...
      for (auto i = 0u; i < s.size(); i += step) {
        if (i == 0)
          ...
          dbgs("  :%02d:", i);
        for (auto j = i; j < i + step && j < s.size(); j++) {
          dbgs(" %08x", s[j]);
        }
        dbgs("\n");
      }
    ...
    case Opc::la: {
      ...
      int from = *eip++;
      stack.back().at(to) =
          ((stack.back().begin() + from) - memory.begin()) * 4;
      dbgs("%p: la %d, %d\n", oldeip, to, from);
    } break;
    case Opc::ld: {
      int to = *eip++;
    ...
    case Opc::st: {
      ...
      if (memory.size() * 4 <= (size_t)to) {
        ...
        uint8_t *from_ptr = (uint8_t *)&stack.back().at(from);
    ...
    case Opc::add: {
      ...
      int lhs = *eip++;
      int rhs = *eip++;
      stack.back().at(to) = stack.back().at(lhs) + stack.back().at(rhs);
      dbgs("%p: add %d, %d, %d\n", oldeip, to, lhs, rhs);
    } break;
    case Opc::sub: {
      int to = *eip++;
    ...
    case Opc::jmp: {
      uint64_t ptrlo = *eip++;
      uint64_t ptrhi = *eip++;
      dbgs("%p: jmp %p\n", oldeip, lohi_to_ptr<void>(ptrlo, ptrhi));
    ...
    case Opc::call: {
      ...
      int ptr = (stack.back().begin() - memory.begin()) + stack.back().size();
...
int Compiler::primary_exp(Program *prog, const std::string &tok, int to) {
  if (tok[0] == '#') {
    if (to == INT_MAX)
      to = newTemp();
    long long value;
...
/* stmt label */
bool Compiler::handle_label(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // LABEL ID :
  if (toks.size() != 3 || toks.at(2) != ":")
    return false;
...
/* stmt func */
bool Compiler::handle_func(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // FUNCTION ID :
  if (toks.size() != 3 || toks.at(2) != ":")
    return false;
...
bool Compiler::handle_assign(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // ID := (ID|NUM|&ID)
  if (toks.size() != 3 || toks.at(0).at(0) == '*' || toks.at(1) != ":=")
    return false;
...
bool Compiler::handle_arith(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // ID := (ID|NUM|&ID) OP (ID|NUM|&ID)
  if (toks.size() != 5 || toks.at(1) != ":=")
    return false;
...
bool Compiler::handle_takeaddr(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // ID := &ID
  if (toks.size() != 3 || toks.at(1) != ":=" || toks.at(2).at(0) != '&')
    return false;
  auto &x = toks.at(0);
...
bool Compiler::handle_deref(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // ID := *ID
  /* stmt assign */
  if (toks.size() != 3 || toks.at(0).at(0) == '*' || toks.at(2).at(0) != '*' ||
      toks.at(1) != ":=")
    return false;
...
bool Compiler::handle_deref_assign(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // *ID := (ID|NUM|&ID)
  if (toks.size() != 3 || toks.at(0).at(0) != '*' || toks.at(1) != ":=")
    return false;
...
bool Compiler::handle_goto_(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // GOTO label
  if (toks.size() != 2)
    return false;
...
bool Compiler::handle_branch(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // IF a < b GOTO label
  ...
  static std::map<std::string, Opc> s2op{
      {"<", Opc::slt},  {">", Opc::sgt},  {"<=", Opc::sle},
      {">=", Opc::sge}, {"==", Opc::seq}, {"!=", Opc::sne},
  };

  if (s2op.find(opc) == s2op.end())
    return false;
...
bool Compiler::handle_ret(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // RETURN exp
  if (toks.size() != 2)
    return false;
...
bool Compiler::handle_dec(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // DEC id num
  if (toks.size() != 3)
    return false;
...
bool Compiler::handle_arg(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // ARG exp
  if (toks.size() != 2)
    return false;
...
bool Compiler::handle_call(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // ID := CALL ID
  if (toks.size() != 4 || toks.at(1) != ":=")
    return false;
  auto &to = toks.at(0);
  auto &f = toks.at(3);
...
bool Compiler::handle_param(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // PARAM ID
  if (toks.size() != 2)
    return false;
...
bool Compiler::handle_read(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // READ ID
  if (toks.size() != 2)
    return false;
...
bool Compiler::handle_write(Program *prog, const TokenList &toks) {
  dbgs("at %s, %d\n", __func__, __LINE__);
  // WRITE ID
  if (toks.size() != 2)
    return false;
...
std::unique_ptr<Program> Compiler::compile(std::istream &is) {
  ...
  while ((is.peek(), is.good())) {
    ...
    prog->gen_inst(Opc::helper, ptr_lo((void *)log_curir),
                   ptr_hi((void *)log_curir), 3, ptr_lo(ir), ptr_hi(ir),
                   lineno);
#endif
    ...
    if (tokens.size() == 0) {
    ...
    } else if (tokens.size() == 4 && tokens[1] == ":=" && tokens[2] == "CALL") {
      ...
      std::vector<bool (Compiler::*)(Program *, const TokenList &)> handlers{
          &Compiler::handle_assign,       &Compiler::handle_arith,
          &Compiler::handle_takeaddr,     &Compiler::handle_deref,
    ...
    fprintf(stderr, "[IGNORED] syntax error at line %d: '%s'\n", lineno,
            line.c_str());
